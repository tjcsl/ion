...
def api_root(request, format=None):
    ...
    views = OrderedDict((("Schedule", {
        ...
        "/schedule/<date>": [
            "Get the schedule for a specific day, in YYYY-MM-DD format", perma_reverse(request, "api_schedule_day_detail",
        ...
        "/profile/<username>":
        ["Get specific user profile by username", perma_reverse(request, "api_user_profile_detail_by_username", kwargs={"username": "2017ewang"})],
        "/profile/<pk>/picture":
        ["Get a specific user's profile picture", perma_reverse(request, "api_user_profile_picture_default", kwargs={"pk": 489})],
        ...
        "/blocks?start_date=<start_date>": [
            "Get a block list starting on the specified date (in YYYY-MM-DD format).",
            "{}?start_date=2015-11-18".format(perma_reverse(request, "api_eighth_block_list"))
        ],
        "/blocks?date=<date>": [
            "Get a list of blocks only on the specified date (in YYYY-MM-DD format).",
        ...
        ["Get favorited eighth activities for current user, or toggle whether an activity is favorited", perma_reverse(request, "api_eighth_user_favorites_list_myid")],
