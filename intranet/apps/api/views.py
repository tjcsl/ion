...
def api_root(request, format=None):  # pylint: disable=redefined-builtin; It doesn't appear we can change this keyword argument name
    ...
    views = OrderedDict((("Schedule", {
        ...
        "/emerg": ["Get FCPS emergency announcement information", perma_reverse(request, "api_emerg_status")]
    }), ("Profile", {
        "/profile": ["Get current user profile", perma_reverse(request, "api_user_myprofile_detail")],
        "/profile/<pk>": ["Get specific user profile by user ID",
        ...
        "/search/<query>": ["Search users (see {}?tips for advanced search documentation)".format(perma_reverse(request, "search")),
        ...
        "/blocks?start_date=<start_date>": [
            "List all blocks starting on the specified date (in YYYY-MM-DD format; paginated)", "{}?start_date=2015-11-18".format(
                perma_reverse(request, "api_eighth_block_list"))
        ],
        "/blocks?date=<date>": [
            "List all blocks on the specified date (in YYYY-MM-DD format)", "{}?date=2015-11-18".format(
                perma_reverse(request, "api_eighth_block_list"))
        ],
        ...
        "/activities/<pk>": ["Get details for a specific eighth period activity (including some scheduling information)",
