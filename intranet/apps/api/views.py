...
def api_root(request, format=None):
    ...
    views = OrderedDict((("Schedule", {
        ...
        "/profile/<pk>/picture": ["Get a specific user's profile picture", perma_reverse(request, "api_user_profile_picture_default", kwargs={"pk":
                                                                                                                                              489})]
    }), ("Classes", {
        ...
        "/blocks?date=<date>": ["Get a list of blocks only on the specified date (in YYYY-MM-DD format).", "{}?date=2015-11-18".format(perma_reverse(
            request, "api_eighth_block_list"))],
        ...
        "/signups/user": ["Get eighth signups for current user, or sign up a user for activity", perma_reverse(request,
                                                                                                               "api_eighth_user_signup_list_myid")],
        "/signups/user/<user_id>": ["Get eighth signups for specific user", perma_reverse(request, "api_eighth_user_signup_list", kwargs={"user_id":
                                                                                                                                          8889})],
        "/signups/scheduled_activity/<scheduled_activity_id>": ["Get eighth signups for a specific scheduled activity", perma_reverse(
            request, "api_eighth_scheduled_activity_signup_list", kwargs={"scheduled_activity_id": 889})]
    })))
