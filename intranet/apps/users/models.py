...
class User(AbstractBaseUser, PermissionsMixin):
    ...
    def last_first_id(self) -> str:
        ...
        """
        return (
            f"{self.last_name}{', ' + self.first_name if self.first_name else ''} "
    ...
    def last_first_initial(self) -> str:
        ...
        Lastname, F [(Nickname)]
        """
        return f"{self.last_name}{', ' + self.first_name[:1] + '.' if self.first_name else ''}" + (f" ({self.nickname})" if self.nickname else "")
...
class Section(models.Model):
    ...
    def __str__(self):
        return f"{self.course.name} ({self.section_id}) - {self.teacher.full_name if self.teacher else 'Unknown'} Pd. {self.period}"
