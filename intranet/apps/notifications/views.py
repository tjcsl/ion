...
@login_required
def gcm_android_post_view(request):
    if not request.user.has_admin_permission("notifications"):
        return redirect("index")
    try:
    ...
    return render(request, "notifications/gcm_android_post.html", context)


@login_required
def gcm_chrome_post_view(request):
    if not request.user.has_admin_permission("notifications"):
        return redirect("index")
    try:
        has_tokens = (settings.GCM_AUTH_KEY and settings.GCM_PROJECT_ID)
    ...
    nc_all = NotificationConfig.objects.exclude(chrome_gcm_token=None)
    context = {
        "nc_all": nc_all,
        "has_tokens": has_tokens
    }
    ...
    if request.method == "POST":
        ...
        for ncid in nc_users:
            ...
            if nc.chrome_gcm_token:
                reg_ids.append(nc.chrome_gcm_token)
                nc_objs.append(nc)
            else:
                fail_ids.append(ncid)
        ...
        if "multicast_id" in resp:
            ...
            messages.error(request, "Failed. {}".format(req.text))
    return render(request, "notifications/gcm_chrome_post.html", context)
