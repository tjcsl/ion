...
def supports_webpush_notifications(user_agent):
    ...
    Returns:
        True if the browser, taken from the user agent, support webpush notifications. False otherwise.
    """
    if (user_agent.browser.family in ("Chrome", "Opera")) and user_agent.browser.version[0] >= 42:
        ...
        "Safari" in user_agent.browser.family
        and user_agent.os.family == "iOS"
        and (user_agent.os.version[0] > 16 or (user_agent.os.version[0] == 16 and user_agent.os.version[1] >= 4))
    ):
        return True
