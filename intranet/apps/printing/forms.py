...
class PrintJobForm(forms.ModelForm):
    ...
    def validate_size(self):
        ...
        if filesize > settings.FILES_MAX_UPLOAD_SIZE:
            raise forms.ValidationError("The file uploaded is above the maximum upload size ({}MB). ".format(settings.FILES_MAX_UPLOAD_SIZE / 1024 /
                                                                                                             1024))

    file = forms.FileField(validators=[validate_size])
    printer = forms.ChoiceField()
    page_range = forms.RegexField(max_length=100,
                                  required=False,
                                  regex=r'^[0-9,\- ]*$',
    ...
    class Meta:
        model = PrintJob
        fields = ["file", "printer", "page_range"]
